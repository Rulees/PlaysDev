...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _init_client(self):
        ...
        if not token:
            raise AnsibleError(
                "token is empty. provide either "
                "`yacloud_token_file` or `yacloud_token` or YC_TOKEN as env-var")
    ...
    def _process_hosts(self):
        ...
        for instance in self.hosts:
            # Step 1: Check if machine is running before doing anything
            if instance["status"] != "RUNNING":
                continue
            # Step 2: Default grouping logic (unchanged)
            ...
            # Step 3: Additional grouping based on "main" and "upstream" labels
            if "main" in instance["labels"]:
                self.inventory.add_group(group="main")
                self.inventory.add_host(instance["name"], group="main")

            if "upstream" in instance["labels"]:
                self.inventory.add_group(group="upstream")
                self.inventory.add_host(instance["name"], group="upstream")
